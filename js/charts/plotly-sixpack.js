...
export function renderControlCharts(type, groups, grandMean, sigma, subSize, outlierSet) { 
    ...
    const pointColors = groups.map(g => {
        if(subSize === 1 && g.ids && g.ids.length > 0) {
            return outlierSet.has(g.ids[0]) ? '#ef4444' : '#3b82f6';
        }
        return '#3b82f6';
    });
    ...
    const trace1 = { x: labels, y: xData, type: 'scatter', mode: 'lines+markers', line: {color: '#3b82f6', width: 1}, marker: {color: pointColors, size: 6} };
    const shapeUCL = { type: 'line', x0: 0, x1: 1, xref: 'paper', y0: ucl, y1: ucl, line: {color:'#ef4444', dash:'dash', width:1} }; 
    const shapeLCL = { type: 'line', x0: 0, x1: 1, xref: 'paper', y0: lcl, y1: lcl, line: {color:'#ef4444', dash:'dash', width:1} }; 
    const shapeCL = { type: 'line', x0: 0, x1: 1, xref: 'paper', y0: grandMean, y1: grandMean, line: {color:'#22c55e', width:1} }; 
    ...
    const rShapeUCL = { type: 'line', x0: 0, x1: 1, xref: 'paper', y0: uclR, y1: uclR, line: {color:'#ef4444', dash:'dash', width:1} }; 
    const rShapeCL = { type: 'line', x0: 0, x1: 1, xref: 'paper', y0: rBar, y1: rBar, line: {color:'#22c55e', width:1} }; 
...
export function renderRunChart(groups, mean, usl, lsl, outlierSet) { 
    ...
    groups.forEach(g => { 
        ...
        values.forEach((v, i) => {
            flatY.push(v);
            const id = ids[i];
            if (id !== undefined && outlierSet && outlierSet.has(id)) { flatColors.push('#ef4444'); } 
            else { flatColors.push('#8b5cf6'); }
        });
    }); 
    
    const trace = { y: flatY, type: 'scatter', mode: 'lines+markers', line: {color: '#8b5cf6', width: 1}, marker: {color: flatColors, size: 5} }; 
    const shapes = [ 
        { type: 'line', x0: 0, x1: 1, xref: 'paper', y0: mean, y1: mean, line: {color:'#22c55e', width:1} }, 
        { type: 'line', x0: 0, x1: 1, xref: 'paper', y0: usl, y1: usl, line: {color: '#ef4444', width: 2, dash: 'dash'} }, 
        { type: 'line', x0: 0, x1: 1, xref: 'paper', y0: lsl, y1: lsl, line: {color: '#ef4444', width: 2, dash: 'dash'} } 
    ]; 
...
export function renderHistogram(data, mean, stdevOverall, stdevWithin, lsl, usl) { 
    const traceHist = { x: data, type: 'histogram', opacity: 0.6, marker: {color: '#60a5fa'}, name: 'Data', histnorm: 'probability density' }; 
    ...
    const traceWithin = { x: xGrid, y: yWithin, type: 'scatter', mode: 'lines', line: {color:'#ef4444', width:2}, name: 'Within' }; 
    
    let yOverall = []; if(stdevOverall > 0) yOverall = xGrid.map(x => jStat.normal.pdf(x, mean, stdevOverall)); 
    const traceOverall = { x: xGrid, y: yOverall, type: 'scatter', mode: 'lines', line: {color:'#3b82f6', width:2, dash:'dot'}, name: 'Overall' }; 
    
    const shapes = [ { type: 'line', y0: 0, y1: 1, yref: 'paper', x0: lsl, x1: lsl, line: {color: '#ef4444', width: 2, dash: 'dash'} }, { type: 'line', y0: 0, y1: 1, yref: 'paper', x0: usl, x1: usl, line: {color: '#ef4444', width: 2, dash: 'dash'} } ]; 
...
export function renderProbPlot(data, adStats) { 
    ...
    const tracePoints = { x: sorted, y: Zscores, type: 'scatter', mode: 'markers', marker: {color:'#3b82f6', size:4} }; 
    const traceLine = { x: lineX, y: lineY, type: 'scatter', mode: 'lines', line: {color:'#ef4444', width:1} }; 
...
export function renderStatsPanel(
  ...
  Ppk_Norm
    ...
    const cpkColor = Cpk >= targetCpk ? "text-green-500" : "text-red-500"; 
    ...
    if (ppkIsPercentile) {
        ...
        ppkDisplay = `<span class="text-2xl font-bold ${Ppk >= targetCpk ? 'text-green-500' : 'text-red-500'}">${Ppk.toFixed(2)}</span>`;
